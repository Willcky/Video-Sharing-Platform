...
public class VideoViewCountServiceImpl implements IVideoViewCountService {
     ...
     */
    private final Map<Long, Long> VIEW_COUNT_MAP = new HashMap<>();
    ...
    public void increaseViewCount(Long videoId) {
        try {
            ...
            Long viewCount = RedisUtils.getCacheObject(redisKey);
            ...
            future.whenComplete((result, ex) -> {
                if (ex == null) {
                    log.info("发送视频播放量消息成功：{}", messageId);
                } else {
    ...
    public void consumeViewCount(VideoViewMessage message, Acknowledgment ack) {
        try {
            ...
            if (ack != null) {
                ack.acknowledge();
                log.info("确认消费消息：messageId={}", message.getMessageId());
            }
        } catch (Exception e) {
    ...
    public void syncViewCount2Db() {
        ...
        try {
            // 构建批量更新对象列表
            List<SysVideo> updateList = currentCounts.entrySet().stream()
                .map(entry -> {
                    SysVideo video = new SysVideo();
                    video.setId(entry.getKey());
                    video.setViewCount(entry.getValue());
                    return video;
                })
                .collect(Collectors.toList());

            // 使用MyBatis-Plus的批量更新
            boolean success = videoMapper.updateBatchById(updateList);
            if (success) {
                log.info("成功更新{}个视频的播放量", updateList.size());
            } else {
